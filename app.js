...
const navTargets = ["features", "splitPanels", "courses", "courses", "diagnosis", "storeInfo"];
...
function renderNav(items) {
  ...
  nav.innerHTML = items.map((label, index) => `<a href="#${navTargets[index] || "storeInfo"}">${safe(label)}</a>`).join("");
}
function renderHero(h) {
  const el = document.getElementById("hero");
  el.style.backgroundImage = `url('${safe(h.image, "assets/hero/beer-garden-hero.jpg")}')`;
  el.innerHTML = `<div class="hero-inner"><div class="badge">${safe(h.period, "期間限定")}</div><div>${safe(h.eyebrow, "SUMMER BEER GARDEN")}</div><h1>${safe(h.title, "屋外ビアガーデン").replace(/\n/g, "<br>")}</h1><p>${safe(h.desc, "")}</p><div class="btn-row"><button class="btn primary" data-action="reserve">${safe(h.ctaPrimary, "予約する")}</button><button class="btn secondary" data-action="availability">${safe(h.ctaSecondary, "空席確認")}</button></div></div>`;
}
...
function renderCourses(list) {
  ...
  el.innerHTML = `<div class='section-head'><h2>おすすめコース</h2></div><div class='cards'>${cards.map((c) => `<article class='course-card'><img src='${safe(c.image, "assets/food/meat-grill.jpg")}' alt='${safe(c.name, "course")}'><div class='card-body'><h3>${safe(c.name, "")}</h3><strong>${safe(c.price, "")}</strong><p>${safe(c.desc, "")}</p><button class='btn' data-action='course'>詳細を見る</button></div></article>`).join("")}</div>`;
}
function renderPanels(list) {
  const el = document.getElementById("splitPanels");
  el.innerHTML = `<div class='cards'>${list.map((p) => `<article class='panel dark'><img src='${safe(p.image)}' alt='${safe(p.title)}'><div class='card-body'><h3>${safe(p.title)}</h3><p>${safe(p.subtitle)}</p><ul>${(p.items || []).map((it) => `<li>${it}</li>`).join("")}</ul><a href='#courses'>${safe(p.link)}</a></div></article>`).join("")}</div>`;
}
function renderScenes(list) {
  const el = document.getElementById("scenes");
  el.innerHTML = `<h2>こんなシーンにおすすめ！</h2><div class='cards'>${list.map((s) => `<article class='scene-card'><img src='${safe(s.image)}' alt='${safe(s.title)}'><div class='card-body'><h3>${safe(s.title)}</h3><p>${safe(s.text)}</p></div></article>`).join("")}</div><div class='closing-cta'><h2>夏の思い出を、ここでつくろう。</h2><div class='btn-row'><button class='btn primary' data-action='reserve'>今すぐ予約する</button><button class='btn secondary' data-action='availability'>空席を確認する</button></div></div>`;
}
...
function renderDiagnosis() {
  ...
  d.innerHTML = `<div class="diagnosis-head"><h2>あなたに合う 肉料理 × 一杯 診断</h2><button class="diagnosis-close" type="button" aria-label="診断を閉じる">×</button></div>
  ...
  <div class='diag-card'><h3>診断結果（デモ）</h3><p>あなたのタイプ：FMDS 肉宴マスター型</p><div class='diag-food'><img src='assets/food/meat-grill.jpg' alt='肉料理と一杯の組み合わせ例'></div><button class='btn primary' data-action='reserve'>この組み合わせで予約する</button></div>`;
}
function setDiagnosis(open) {
  const aside = document.getElementById("diagnosis");
  const toggle = document.getElementById("sidebarToggle");
  aside.classList.toggle("open", open);
  toggle.setAttribute("aria-expanded", String(open));
  toggle.setAttribute("aria-label", open ? "診断を閉じる" : "診断を開く");
}
function showDemoStatus(message) {
  const status = document.getElementById("demoStatus");
  status.textContent = message;
  status.classList.add("show");
  window.clearTimeout(showDemoStatus.timer);
  showDemoStatus.timer = window.setTimeout(() => status.classList.remove("show"), 2800);
}
function wireInteractions() {
  const toggle = document.getElementById("sidebarToggle");
  const aside = document.getElementById("diagnosis");
  toggle.addEventListener("click", () => setDiagnosis(!aside.classList.contains("open")));
  ...
  document.addEventListener("click", (event) => {
    ...
    if (action === "reserve") showDemoStatus("予約導線のデモです。公開時に予約URLへ接続します。");
    if (action === "availability") showDemoStatus("空席確認のデモです。公開時に確認先へ接続します。");
    if (action === "course") showDemoStatus("コース詳細のデモです。JSONの詳細情報へ接続予定です。");
  });
}
function wireImageFallbacks() {
  document.addEventListener("error", (event) => {
    ...
    event.target.parentElement?.classList.add("image-placeholder");
  }, true);
}
